 ...
 */
export async function runAgentPhase(agentName: string, allowedFiles: string[]) {
    ...
    if (!fs.existsSync(agentPath)) throw new Error(`Agent ${agentName} not found.`);
    ...
    const contextMap = allowedFiles.map(fileName => {
        ...
        if (!fs.existsSync(filePath)) return `### FILE: ${fileName}\n(Not found)`;
...
async function callAIProvider(payload: { system: string, prompt: string }) {
